...
A função `c()` \indt{vetores} \indf{c} combina valores que formam vetores^[Vetores são uma estrutura de dados básica do R que permite armazenar um conjunto de valores numéricos ou de caracteres em um objeto nomeado]. Abaixo, é demonstrado como vetores podem ser criados utilizando `c()`. Note que o código é dado entre parênteses `(...)` para que o valor seja armazenado no ambiente ao mesmo tempo em que é impresso no console.
