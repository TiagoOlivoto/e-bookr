...
O conjunto de dados "modelagem" possui todas as combinações (genótipo *vs* ambiente) com o número de repetições informado no argumento `nrepval`. O conjunto de dados "validação" tem uma repetição. O diagrama abaixo representa o procedimento realizado.
...
```{r}
AMMIF = cv_ammif(data_ge, ENV, GEN, REP, GY, nboot = 20, nrepval = 2)
```
...
Os seguintes índices de estabilidade baseados em AMMI podem ser calculados usando a função `AMMI_indexes()`:
...
```{r echo = TRUE}
BLUP_model = waasb(data_ge, ENV, GEN, REP, GY, verbose = FALSE)
...
```{r }
valida_blup = cv_blup(data_ge, ENV, GEN, REP, GY, nboot = 20, nrepval = 2)
