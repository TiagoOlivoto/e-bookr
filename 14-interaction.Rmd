...
```{r, fig.width = 10, fig.height = 5, aviso = FALSE, mensagem = FALSE}
...
Como exemplo, a função `cv_ammi()`\indf{cv\_ammi} é usada para calcular um procedimento de validação cruzada para os modelos AMMI0, AMMI2 e AMMIF (9 eixos).
```{r echo = FALSE}
load("cvali.RData")
```
...
* **AMMI0 para AMMIF**
A função `cv_ammif()`\indf{cv\_ammif} é usada para calcular um procedimento de validação cruzada para todos os membros do modelo da família AMMI. Nesse caso, AMMI0-AMMI9.

```{r eval = FALSE}
AMMIF <- cv_ammif(data_ge, ENV, GEN, REP, GY)
```

* **Validação cruzada para previsão de BLUP**
A função `cv_blup ()` fornece uma validação cruzada de dados baseados em replicação usando modelos mistos. Por padrão, blocos completos são selecionados aleatoriamente para cada ambiente. Usando o argumento `random ', é possível escolher os efeitos aleatórios do modelo, como mostrado abaixo.
...
```{r eval = TRUE, fig.height = 5, fig.width = 10, fig.align =" center ", mensagem = F, aviso = F}
